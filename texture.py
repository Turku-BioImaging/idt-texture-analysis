...
def haralick(image: np.ndarray, mask: np.ndarray):
    ...
    features = mahotas.features.haralick(props[0]['intensity_image'], distance=1, ignore_zeros=True)
    ...
    for index in range(len(__HARALICK_FEATURES)):
        feature_averages[__HARALICK_FEATURES[index]] = np.mean([features[i][index] for i in range(4)])
